split_and_fit = function(region,
  ...
  if(split_crit == "y_min_minus_a_max") {
    print(sprintf("Updating run_obs_vec and best_so_far_vec from region_1_bo"))
    first_NA_index <- min(which(is.na(run_obs_vec)))
    ...
    print(sprintf("Updating run_obs_vec and best_so_far_vec from region_2_bo"))
    first_NA_index <- min(which(is.na(run_obs_vec)))
  ...
  print(sprintf("Returning region_1 and region_2 and other data"))
