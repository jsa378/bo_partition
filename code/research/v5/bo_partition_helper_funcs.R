...
prep_subregions <- function(region,
  ...
  region_1_x = region_x[region_x[, split_dimension] < (split_point + point_share_tol), ]
  region_1_y = region_y[which(region_x[, split_dimension] < (split_point + point_share_tol))]
  
  region_2_x = region_x[region_x[, split_dimension] >= (split_point - point_share_tol), ]
  region_2_y = region_y[which(region_x[, split_dimension] >= (split_point - point_share_tol))]
  ...
  for (d in 1:dimension) {
    ...
    for (p in 1:length(percentiles)) {
      ...
      subregions <- prep_subregions(region = region,
                                    ...
                                    point_share_tol = point_share_tol_param
                                    )
      ...
      # Print out which points the subregions share in common

      region_1_x_and_y <- as.data.frame(cbind(region_1$region_x, region_1$region_y))
      region_2_x_and_y <- as.data.frame(cbind(region_2$region_x, region_2$region_y))
      common_points <- inner_join(region_1_x_and_y, region_2_x_and_y)

      if (nrow(common_points) > 0) {

        print("The two subregions share the following points in common:")
        print("(The right-most column is the objective function values)")
        print(common_points)
