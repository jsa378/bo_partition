prep_epsilon_vec <- function(region,
                             epsilon) {
  
  print("The region bounds are:")
  print(region$bound_matrix)
  
  lower_bounds <- region$bound_matrix[, 1]
  upper_bounds <- region$bound_matrix[, 2]
  
  length_vec <- upper_bounds - lower_bounds
  
  print("The region lengths are:")
  print(length_vec)
  
  epsilon_vec <- length_vec * epsilon
  
  print("The epsilon values are:")
  print(epsilon_vec)
...
augment_points <- function(all_x,
  ...
  for (point in 1:num_pts) {
    ...
    is_point_in_aug_reg[point, 1] <- all(between(all_x[point, ],
                                       aug_lower_bounds,
                                       aug_upper_bounds
                                       )
                               )
...
explore_region <- function(region,
  ...
  all_x_mat <- update$all_x
  all_y_mat <- update$all_y
  ...
  while (n < n_max) {
    ...
    print("Preparing the augmented points for the Gaussian process model")
    
    epsilon_vec <- prep_epsilon_vec(region = region,
                                    epsilon = epsilon_param)
    
    augmented_points <- augment_points(all_x = all_x_mat,
                                       all_y = all_y_mat,
                                       region = region,
                                       epsilon_vec = epsilon_vec)
    
    aug_x_pts <- augmented_points$aug_x
    aug_y_pts <- augmented_points$aug_y
    ...
    gp_model <- km(
      formula = ~1,
      design = aug_x_pts, # region_x,
      response = aug_y_pts, # region_y,
...
split_and_fit <- function(region,
  ...
  region_1_epsilon_vec <- prep_epsilon_vec(region = region_1_return,
                                           epsilon = epsilon_param)
  ...
  region_1_gp_model <- km(
    formula = ~1,
    design = region_1_aug_x_pts, # region_1_return$region_x,
    response = region_1_aug_y_pts, # region_1_return$region_y,
    covtype = covtype_param,
    nugget = nugget_param,
    control = c(dice_ctrl, trace = FALSE),
    optim.method = "gen"
  ...
  region_2_epsilon_vec <- prep_epsilon_vec(region = region_2_return,
                                           epsilon = epsilon_param)
  
  region_2_augmented_points <- augment_points(all_x = all_x_mat,
                                              all_y = all_y_mat,
                                              region = region_2_return,
                                              epsilon_vec = region_2_epsilon_vec)
  
  region_2_aug_x_pts <- region_2_augmented_points$aug_x
  region_2_aug_y_pts <- region_2_augmented_points$aug_y
  ...
  region_2_gp_model <- km(
    formula = ~1,
    design = region_2_aug_x_pts, # region_2_return$region_x,
    response = region_2_aug_y_pts, # region_2_return$region_y,
    covtype = covtype_param,
    nugget = nugget_param,
    control = c(dice_ctrl, trace = FALSE),
    optim.method = "gen"
