...
if (working == "remote") {
  ...
  epsilon_param <- as.numeric(args[11])
  ...
  # source("/Users/jesse/Downloads/bo_partition/code/research/v6/bo_partition_helper_funcs.R")
...
paste(c("Partition-Bayesian optimization v6 (include points within epsilon of region bounds for Gaussian process model)"), collapse = " ")
paste(c("Seed value:", seed_value), collapse = " ")
...
paste(c("epsilon_param:", epsilon_param), collapse = " ")
...
all_x_pts[(1:num_init_obs), ] <- as.matrix(init_points)
all_y_vals[(1:num_init_obs), 1] <- as.matrix(init_y)
...
while (length(all_regions) > 0) {
  ...
  all_x_pts <- results$all_x
  all_y_vals <- results$all_y
